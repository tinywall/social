...
class Status extends TW_Controller{
	...
	function posts(){
		if($this->userValid){
			$result=$this->twfunctions->get_status_posts($this->current_user->id_users,$this->uri->segment(4));
			$this->status_result=$result['status'];
			$this->comment_result=$result['comment'];
			$this->_json_status($this->status_result);
		}
	...
	function _json_status($status_result){
		...
		foreach($status_result as $row){
			...
			$arr2=array('id'=>$row->id_status_post,'from'=>$arr,'message'=>$row->status_message,'status_image'=>$row->image,'status_link'=>$row->link,'status_video'=>$row->video,'type'=>$row->type,'like_count'=>$row->like_count,'liked'=>$row->liked,'commentcount'=>(string)(sizeof($comres)),'comments'=>$this->_json_comment($comres),'created_at'=>date('Y-m-d\TH:i:s\+0000',strtotime($row->time)));
	...
	function postStatus(){
		if($this->userValid){
			...
			if($status_user_data=$this->twfunctions->getCurrentDataPublic($status_owner)){
				...
				if($result=$this->twfunctions->post_new_status($this->session_user->id_users,$data)){
					if($status_user->id_users!=$this->session_user->id_users){
						$this->send_twmail($status_user->email,'','Wall post from '.$this->session_user->first_name.' '.$this->session_user->last_name,$this->session_user->first_name.' '.$this->session_user->last_name.' posted a message on your wall.<br/><br/>'.$data['message']);
					}
					$this->status_result=$result['status'];
					$this->comment_result=$result['comment'];
					$this->_json_status($this->status_result);
				}else{
					$outarr=array('message'=>"Something went wrong",'success'=>FALSE);
					header('Content-type: application/json');
